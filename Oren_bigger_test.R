...
dem.panel <- fread("dem.panel.tab")


#'run_sim' - run all the simulations and save the results
#'load' - don't run the simulationd and just load the results
mode <- "load"
...
non.comp <- unique(subset(dem.panel, base.poll > 70 | base.poll < 30
                          | is.na(base.poll) ,
                          "race"))[,1]
nc_vec <- !(dem.panel$race %in% non.comp$race)
dem.panel <- dem.panel[nc_vec,]
...
check_na <- c(unique(xvars.denom.trees, xvars.numer.trees))
dem.panel$missing <- aaply(is.na(dem.panel[,..check_na]), 1, any)
...
rows <- dim(subData)[1]
test <- sample(1:rows, floor(1/4*rows), replace=FALSE)
length(test)
...
if (mode == "run_sim"){
  cvList <- llply(1:5, oneFold, .folds=3, thisData=dem.panel.train, .parallel=TRUE)
  saveRDS(cvList, "cvList.rds")
  beep()
} else if (mode == "load"){
  cvList <- readRDS("cvList.rds")
}

rmse.cvStudy <- laply(cvList, function(z) mean(c((z-dem.panel.train[, "d.gone.neg"])^2), na.rm = TRUE))

bestParam.cvStudy <- param.grid[which(rmse.cvStudy==min(rmse.cvStudy)),] ## this is the parameter grid we will use

#required for replication - too computationally expensive to calculate
bestParam.cvStudy$trees <- 638
...
dem.panel.test$bart.pred <- dem.panel.test$gbm.pred <- dem.panel.test$gam.pred <- 0
...
if (mode == "run_sim"){
  bart.dem <- bart(x.train=dem.panel.train[!missing.train,..xvars.denom.trees],
                   y.train=as.factor(dem.panel.train[!missing.train,..yvar]$d.gone.neg),
                   x.test=dem.panel.test[!missing.test,..xvars.denom.trees], ndpost=20000, nskip=10000)
  saveRDS(bart.dem, "bart.dem.rds")
  beep()
} else if (mode == "load"){
  bart.dem <- readRDS("bart.dem.rds")
}
...
#table 5.2
outMat<-matrix(NA, nrow=3, ncol=6, dimnames = 
                 (list(c("GAM", "GBM", "BART"), 
                       c("Accuracy", "RMSE", "AUROC", "AUSen", "AUSpec", "LogLoss"))))

classpreds.gam <- dem.panel.test$gam.pred >= 0.5
classpreds.gbm <- dem.panel.test$gbm.pred >= 0.5
classpreds.bart <- dem.panel.test$bart.pred >= 0.5

outMat[1,1] <- mean(dem.panel.test$d.gone.neg == classpreds.gam, na.rm=TRUE)
outMat[2,1] <- mean(dem.panel.test$d.gone.neg == classpreds.gbm, na.rm=TRUE)
outMat[3,1] <- mean(dem.panel.test$d.gone.neg == classpreds.bart, na.rm=TRUE)

outMat[1,2] <- sqrt(mean(errors.gam$d.gone.neg^2,na.rm=TRUE))
outMat[2,2] <- sqrt(mean(errors.gbm$d.gone.neg^2, na.rm=TRUE))
outMat[3,2] <- sqrt(mean(errors.bart$d.gone.neg^2, na.rm=TRUE))

sqrt(mean(errors.bart$d.gone.neg^2, na.rm=TRUE))

outMat[1,3] <-auc(roc(dem.panel.test$gam.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[2,3] <-auc(roc(dem.panel.test$gbm.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[3,3] <-auc(roc(dem.panel.test$bart.pred, labels=as.factor(dem.panel.test$d.gone.neg)))

outMat[1,4] <- auc(sensitivity(dem.panel.test$gam.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[2,4] <- auc(sensitivity(dem.panel.test$gbm.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[3,4] <- auc(sensitivity(dem.panel.test$bart.pred, labels=as.factor(dem.panel.test$d.gone.neg)))

outMat[1,5] <- auc(specificity(dem.panel.test$gam.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[2,5] <- auc(specificity(dem.panel.test$gbm.pred, labels=as.factor(dem.panel.test$d.gone.neg)))
outMat[3,5] <- auc(specificity(dem.panel.test$bart.pred, labels=as.factor(dem.panel.test$d.gone.neg)))

outMat[1,6] <- LogLoss(dem.panel.test$d.gone.neg, dem.panel.test$gam.pred)
outMat[2,6] <- LogLoss(dem.panel.test$d.gone.neg, dem.panel.test$gbm.pred)
outMat[3,6] <- LogLoss(dem.panel.test$d.gone.neg, dem.panel.test$bart.pred)

xtable(outMat, digits=3)

write.csv(dem.panel.train, "train.csv")
write.csv(dem.panel.test, "test.csv")
'
## Now calculate the fit statistics for a 5-fold CV within the training set alone
withinTraining <- function(.folds=5, thisData, params){
  set.seed(54321)
  folds.index <- sample(as.numeric(cut(1:nrow(thisData),.folds)))
  ...
  for(this.fold in 1:.folds) {
    ...
    bart.dem <- bart(x.train=thisData[inSample,..xvars.denom.trees],
                     y.train=as.factor(unlist(thisData[inSample,..yvar])),
                     x.test=thisData[outSample,..xvars.denom.trees], ndpost=4000, nskip=2000
                     ,verbose=TRUE)
...
if (mode == "run_sim"){
  cvWithinTraining<-withinTraining(.folds=5
                                   , thisData=dem.panel.train[dem.panel.train$pos.prob.subset & !missing.train,]
                                   , params=bestParam.cvStudy)
  
  saveRDS(cvWithinTraining, "cvWithinTraining.rds")
  beep()
} else if (mode == "load"){
  cvWithinTraining <- readRDS("cvWithinTraining.rds")
}
...
outMat2[1,2] <- sqrt(mean(errors.gam.train$d.gone.neg^2 , na.rm=TRUE))
outMat2[2,2] <- sqrt(mean(errors.gbm.train$d.gone.neg^2 , na.rm=TRUE))
outMat2[3,2] <- sqrt(mean(errors.bart.train$d.gone.neg^2 , na.rm=TRUE))
...
iptw.bart <- function(y, xvars.denominator, xvars.numerator, data, id, time, ones = NULL, missing) {
  ...
  dem <- bart(x.train=data[!ones & !missing,..xvars.denominator],
              y.train=as.factor(unlist(data[!ones & !missing,..y])), ndpost=4000, nskip=2000,verbose=TRUE)
...
if (mode == "run_sim"){
  ...
  saveRDS(bart.fit, "bart.fit.rds")
} else if (mode == "load"){
  bart.fit <- readRDS("bart.fit.rds")
}
...
plot(density(unlist(dem.final[,c("gam.sw")])), xlab="", main="")
rug(unlist(dem.final[,c("gam.sw")]))
title(line=.5, main="GAM weights")
plot(density(unlist(dem.final[,c("gbm.sw")]), na.rm=TRUE), xlab="", main="")
rug(unlist(dem.final[,c("gbm.sw")]))
title(line=.5, main="GBM weights")
plot(density(unlist(dem.final[,c("bart.sw")]), na.rm=TRUE), xlab="", main="")
rug(unlist(dem.final[,c("bart.sw")]))
